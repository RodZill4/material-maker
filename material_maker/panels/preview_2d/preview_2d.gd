...
func update_material(source):
	do_update_material(
		source, material, shader_context_defs + get_shader_custom_functions() + shader
	)
...
func set_generator(g: MMGenBase, o: int = 0, force: bool = false) -> void:
	...
	if is_instance_valid(g):
		...
		if get_node_or_null("ContextMenu") != null:
			$ContextMenu.set_item_disabled(
				$ContextMenu.get_item_index(MENU_EXPORT_ANIMATION), false
			)
	else:
		generator = null
		if get_node_or_null("ContextMenu") != null:
...
func create_image(renderer_function: String, params: Array, size: int) -> void:
	...
	tmp_material.shader.code = MMGenBase.generate_preview_shader(
