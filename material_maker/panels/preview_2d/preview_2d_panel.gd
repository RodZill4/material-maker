...
func get_shader_custom_functions():
	return (
		"vec4 preview_2d_postprocessed(vec2 uv) { return %s; }\n"
...
func update_material(source):
	...
	if temporal_aa_current:
		do_update_material(
			source, $Accumulate/Iteration.material, shader_context_defs + shader_accumulate
		)
...
func value_to_pos(value: Vector2) -> Vector2:
	return (
		(value - center + Vector2(0.5, 0.5)) * min(rect_size.x, rect_size.y) / scale
		+ 0.5 * rect_size
	)
...
func pos_to_value(pos: Vector2) -> Vector2:
	return (
		(pos - 0.5 * rect_size) * scale / min(rect_size.x, rect_size.y)
		+ center
		- Vector2(0.5, 0.5)
	)
