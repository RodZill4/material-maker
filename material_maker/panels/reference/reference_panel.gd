	...
	if event is InputEventMouseButton:
		if event.pressed:
			if event.button_index == BUTTON_LEFT and selected_slot != null:
				...
				$ContextMenu.popup(
					Rect2(get_global_mouse_position(), $ContextMenu.get_minimum_size())
				)
		elif event.button_index == BUTTON_MIDDLE:
			dragging = false
		...
		if dragging:
			...
			if gradient_length > 0.0:
				for i in range(gradient.get_point_count()):
					gradient.set_point_position(
						i, gradient.get_point_position(i) * gradient_length / new_gradient_length
					)
