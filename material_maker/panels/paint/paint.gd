...
func set_settings(s: Dictionary):
	...
	if (
		s.has("texture_size")
		and (s.texture_size is int or s.texture_size is float)
		and s.texture_size != settings.texture_size
	):
	...
	if (
		s.has("bump_strength")
		and s.bump_strength is float
		and s.bump_strength != settings.bump_strength
	):
	...
	if settings_changed:
		...
		layers.set_normal_options(
			settings.paint_normal, settings.paint_depth_as_bump, settings.bump_strength
		)
...
func _on_MaskSelector_pressed():
	...
	var result = dialog.ask(
		{mesh = painted_mesh.mesh, idmap_filename = idmap_filename, mask = mask}
	)
...
func __input(ev: InputEvent):
	if ev is InputEventKey:
		if ev.scancode == KEY_CONTROL:
			...
			or ev.scancode == KEY_RIGHT
			or ev.scancode == KEY_UP
			or ev.scancode == KEY_DOWN
		):
	...
	if ev is InputEventMouseMotion:
		...
		if ev.button_mask & BUTTON_MASK_LEFT != 0:
			if ev.shift:
				...
				painter.update_brush_params(
					{brush_size = brush_size, brush_hardness = brush_hardness}
				)
				$VSplitContainer/HSplitContainer/Painter/Options/Grid/BrushSize.set_value(
					brush_size
				)
				$VSplitContainer/HSplitContainer/Painter/Options/Grid/BrushHardness.set_value(
					brush_hardness
				)
			elif ev.control:
				reset_stroke()
				...
				painter.update_brush_params(
					{pattern_scale = pattern_scale, pattern_angle = pattern_angle}
				)
			...
			if ev.button_index == BUTTON_LEFT:
				if ev.pressed:
					...
					if current_tool == MODE_LINE:
						...
						last_painted_position = (
							mouse_position
							+ Vector2(brush_spacing_control.value, brush_spacing_control.value)
						)
...
func _on_View_gui_input(ev: InputEvent):
	handle_stroke_input(
		ev, PAINTING_MODE_TEXTURE_FROM_VIEW if paint_engine_button.pressed else PAINTING_MODE_VIEW
	)
	if ev is InputEventMouseMotion:
		if ev.button_mask & BUTTON_MASK_MIDDLE != 0:
			if ev.shift:
				...
				camera.translate(
					Vector3(0.0, 0.0, -0.01 * ev.relative.y * camera.transform.origin.z)
				)
				update_view()
				accept_event()
			else:
...
func _on_Texture_gui_input(event):
	...
	var offset_from_center: Vector2 = (
		get_global_mouse_position()
		- (image_rect.position + 0.5 * image_rect.size)
	)
...
func update_procedural_layer() -> void:
	if (
		layers.selected_layer != null
		and layers.selected_layer.get_layer_type() == Layer.LAYER_PROC
		and !procedural_update_changed_scheduled
	):
...
func paint(
	...
	painting_mode: int = PAINTING_MODE_VIEW,
	end_of_stroke: bool = false
):
	if layers.selected_layer == null or layers.selected_layer.get_layer_type() == Layer.LAYER_PROC:
		return
...
func initialize_debug_selects():
	for s in [
		$VSplitContainer/HSplitContainer/Painter/Debug/Select1,
		$VSplitContainer/HSplitContainer/Painter/Debug/Select2
	]:
		s.clear()
		var index = 0
...
func _on_DebugSelect_item_selected(ID, t):
	var texture = [
		$VSplitContainer/HSplitContainer/Painter/Debug/Texture1,
		$VSplitContainer/HSplitContainer/Painter/Debug/Texture2
	][t]
	for p in [self, $Painter, $PaintLayers]:
		var textures_count = p.debug_get_texture_names().size()
		if ID < textures_count:
