...
const CHANNELS: Array = [
	"albedo", "metallic", "roughness", "emission", "normal", "depth", "occlusion"
]
...
func move_layer_into(layer: Layer, target_layer: Layer, index: int = -1) -> void:
	assert(layer != null)
	if (
		layer.get_layer_type() == Layer.LAYER_MASK
		and (target_layer == null or target_layer.get_layer_type() == Layer.LAYER_MASK)
	):
		return
...
func get_shaders(mask_count: int) -> Dictionary:
	while shaders.size() <= mask_count:
		...
		metallic_shader.code = (
			shader_prefix
			+ "	COLOR=vec4(tex.r, tex.r, tex.r, tex.b*"
			+ shader_modulate
			+ ");\n}"
		)
		roughness_shader.code = (
			shader_prefix
			+ "	COLOR=vec4(tex.g, tex.g, tex.g, tex.a*"
			+ shader_modulate
			+ ");\n}"
		)
		albedomask_shader.code = (
			shader_prefix
		...
		shaders.push_back(
			{
				...
				metallic = metallic_shader,
				roughness = roughness_shader,
				albedomask = albedomask_shader
			}
		)
