...
func update_category_button(category: String) -> void:
	if library_manager.is_section_enabled(category):
		...
		category_buttons[category].hint_tooltip = (
			category
...
func add_item(
	...
	if slash_position == -1:
		...
		if new_parent == null:
			...
			new_parent.collapsed = (
				!force_expand
				and expanded_items.find(get_item_path(new_parent)) == -1
			)
	...
	while item != null:
		if item.get_metadata(0) != null:
			...
			image = image.get_rect(
				Rect2(
					new_nodes[0].rect_global_position - Vector2(1, 2),
					new_nodes[0].rect_size + Vector2(4, 4)
				)
			)
			print(get_icon_name(get_item_path(item)))
			image.save_png(
				(
					"res://material_maker/doc/images/node_"
					+ get_icon_name(get_item_path(item))
					+ ".png"
				)
			)
			for n in new_nodes:
	...
	match index:
		0:  # Rename
			...
			var status = dialog.enter_text(
				"Rename item", "Enter the new name for this item", item_path
			)
			while status is GDScriptFunctionState:
				status = yield(status, "completed")
			if status.ok:
		...
		4:  # Define aliases
			...
			add_child(dialog)
			var status = dialog.enter_text(
				"Library item aliases", "Updated aliases for " + item_path, aliases
			)
			while status is GDScriptFunctionState:
				status = yield(status, "completed")
			if !status.ok:
