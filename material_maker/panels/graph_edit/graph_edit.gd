...
func do_zoom(factor: float):
	...
	zoom *= factor
	var position = offset_from_global_position(
	...
	for c in get_children():
		if c is GraphNode:
			...
			if rect.has_point(get_global_mouse_position()) and output_count > 0:
				...
				var output_2: Vector2 = (
					c.get_connection_output_position(output_count - 1)
					+ 5 * scale
				)
					...
					for i in range(output_count):
						if (c.get_connection_output_position(i) - pos).length() < 5 * scale.x:
							if pressed:
								...
								set_current_preview(
									1 if Input.is_key_pressed(KEY_SHIFT) else 0,
									port_click_node,
									port_click_port_index,
	...
	if (
			...
			for c in get_children():
				...
				if rect.has_point(get_global_mouse_position()):
					if c.has_method("get_input_slot"):
						var slot = c.get_input_slot(
							get_global_mouse_position() - c.rect_global_position
						)
						if slot >= 0:
		...
		for c in get_children():
			if c.has_method("get_slot_tooltip"):
				...
				rect = Rect2(rect.position, rect.size * c.get_global_transform().get_scale())
				if rect.has_point(get_global_mouse_position()):
					hint_tooltip = c.get_slot_tooltip(
						get_global_mouse_position() - c.rect_global_position
					)
				else:
...
func connect_node(from: String, from_slot: int, to: String, to_slot: int):
	...
	if (
		out_ports[from_slot].has("group_size")
		and in_ports[to_slot].has("group_size")
		and out_ports[from_slot].group_size == in_ports[to_slot].group_size
	):
		connect_count = out_ports[from_slot].group_size
	var connect_list: Array = []
	var disconnect_list: Array = []
	for i in range(connect_count):
		if generator.connect_children(
			from_node.generator, from_slot + i, to_node.generator, to_slot + i
		):
			var disconnect = get_source(to, to_slot + i)
			if !disconnect.empty():
				.disconnect_node(disconnect.node, disconnect.slot, to, to_slot + i)
				disconnect_list.push_back(
					{
						from = get_node(disconnect.node).generator.name,
						from_port = disconnect.slot,
						to = get_node(to).generator.name,
						to_port = to_slot + i
					}
				)
			...
			connect_list.push_back(
				{
					from = get_node(from).generator.name,
					from_port = from_slot + i,
					to = get_node(to).generator.name,
					to_port = to_slot + i
				}
			)
	...
	if connected:
		...
		var undo_actions = [
			...
			{
				type = "add_to_graph",
				parent = generator_hier_name,
				generators = [],
				connections = disconnect_list
			}
		]
		var redo_actions = [
			{
...
func disconnect_node(from: String, from_slot: int, to: String, to_slot: int) -> void:
	...
	if do_disconnect_node(from, from_slot, to, to_slot):
		var generator_hier_name: String = generator.get_hier_name()
		var connection = {
			from = from_gen.name, from_port = from_slot, to = to_gen.name, to_port = to_slot
		}
		var undo_actions = [
			{
				type = "add_to_graph",
...
func new_material(
	init_nodes = {
		nodes = [{name = "Material", type = "material", "parameters": {"size": 11}}],
		connections = []
	}
) -> void:
	clear_material()
...
func do_create_nodes(data, position: Vector2 = Vector2(0, 0)) -> Array:
	...
	if (
		data.has("nodes")
		and typeof(data.nodes) == TYPE_ARRAY
		and data.has("connections")
		and typeof(data.connections) == TYPE_ARRAY
	):
		var new_stuff = mm_loader.add_to_gen_graph(
			generator, data.nodes, data.connections, position
		)
...
func do_paste(data) -> void:
	...
	if Rect2(Vector2(0, 0), rect_size).has_point(get_local_mouse_position()):
		position = offset_from_global_position(
			get_global_transform().xform(get_local_mouse_position())
		)
	for c in get_children():
		if c is GraphNode:
...
func can_drop_data(_position, data) -> bool:
	return (
		typeof(data) == TYPE_COLOR
		or (
			typeof(data) == TYPE_DICTIONARY
			and (data.has("type") or (data.has("nodes") and data.has("connections")))
		)
	)
...
func highlight_connections() -> void:
	...
	for c in get_connection_list():
		set_connection_activity(
			...
			c.to,
			c.to_port,
			1.0 if get_node(c.from).selected or get_node(c.to).selected else 0.0
		)
...
func set_current_preview(slot: int, node, output_index: int = 0, locked = false) -> void:
	...
	if locked:
		if (
			node != null
			and locked_preview[slot] != null
			and locked_preview[slot].generator != node.generator
		):
			old_locked_preview = locked_preview[slot].generator
		if (
			locked_preview[slot] != null
			and preview != null
			and locked_preview[slot].generator == preview.generator
			and locked_preview[slot].output_index == preview.output_index
		):
			locked_preview[slot] = null
		else:
		...
		if (
			node != null
			and current_preview[slot] != null
			and current_preview[slot].generator != node.generator
		):
...
func request_popup(
	node_name: String, slot_index: int, _release_position: Vector2, connect_output: bool
) -> void:
...
func undoredo_command(command: Dictionary) -> void:
	match command.type:
		"add_to_graph":
			...
			var new_stuff = mm_loader.add_to_gen_graph(
				parent_generator, generators, connections, position
			)
...
func undoredo_move_node(node_name: String, old_pos: Vector2, new_pos: Vector2):
	if old_pos == new_pos:
		return
	var undo_action = {
		type = "move_generators",
		parent = generator.get_hier_name(),
		positions = {node_name: old_pos}
	}
	var redo_action = {
		type = "move_generators",
		parent = generator.get_hier_name(),
		positions = {node_name: new_pos}
	}
...
func set_node_parameters(generator, parameters: Dictionary):
	...
	if !prev_params.empty():
		var undo_action = {
			type = "setparams", node = generator.get_hier_name(), params = prev_params
		}
		var redo_action = {
			type = "setparams", node = generator.get_hier_name(), params = parameters
		}
...
func undoredo_step_actions(
	parent_path: String, prev: Dictionary, next: Dictionary, top: bool = true
) -> Dictionary:
			...
			for pcn in prev.connections.keys():
				...
				if next.connections.has(pcn):
					...
					for pcpn in pc.keys():
						...
						if nc.has(pcpn):
							...
							if pcp.from != ncp.from or pcp.from_port != ncp.from_port:
								undo_add_connections.push_back(
									{
										from = pcp.from,
										from_port = pcp.from_port,
										to = pcn,
										to_port = pcpn
									}
								)
								redo_remove_connections.push_back(
									{
										from = pcp.from,
										from_port = pcp.from_port,
										to = pcn,
								...
								undo_remove_connections.push_back(
									{
										from = ncp.from,
										from_port = ncp.from_port,
										to = pcn,
										to_port = pcpn
									}
								)
								redo_add_connections.push_back(
									{
										from = ncp.from,
										from_port = ncp.from_port,
										to = pcn,
					...
					for pcpn in pc.keys():
						var pcp = pc[pcpn]
						undo_add_connections.push_back(
							{from = pcp.from, from_port = pcp.from_port, to = pcn, to_port = pcpn}
						)
						redo_remove_connections.push_back(
							{from = pcp.from, from_port = pcp.from_port, to = pcn, to_port = pcpn}
						)
			...
			for ncn in next.connections.keys():
				...
				if prev.connections.has(ncn):
					...
					for ncpn in nc.keys():
						var ncp = nc[ncpn]
						undo_remove_connections.push_back(
							{from = ncp.from, from_port = ncp.from_port, to = ncn, to_port = ncpn}
						)
						redo_add_connections.push_back(
							{from = ncp.from, from_port = ncp.from_port, to = ncn, to_port = ncpn}
						)
				...
				undo_actions.push_back(
					{
						type = "remove_connections",
						parent = parent_path,
						connections = undo_remove_connections
					}
				)
			if !redo_remove_connections.empty():
				redo_actions.push_back(
					{
						type = "remove_connections",
						parent = parent_path,
						connections = redo_remove_connections
					}
				)
			if !undo_remove_nodes.empty():
				undo_actions.push_back(
					{
						type = "remove_generators",
						parent = parent_path,
						generators = undo_remove_nodes
					}
				)
				...
				undo_actions.push_back(
					{
						type = "add_to_graph",
						parent = parent_path,
						generators = undo_add_nodes,
						connections = undo_add_connections
					}
				)
				...
				redo_actions.push_back(
					{
						type = "add_to_graph",
						parent = parent_path,
						generators = redo_add_nodes,
						connections = redo_add_connections
					}
				)
		"remote", "ios":
			var generator_path = parent_path.left(parent_path.rfind("/"))
			undo_actions.push_back(
				{type = "update_generator", parent = generator_path, name = next.name, data = prev}
			)
			redo_actions.push_back(
				{type = "update_generator", parent = generator_path, name = prev.name, data = next}
			)
		_:
...
func undoredo_create_step(
	action_name: String, parent_path: String, prev: Dictionary, next: Dictionary
) -> void:
	if prev.type != next.type:
...
func add_reroute_to_input(node: MMGraphNodeMinimal, port_index: int) -> void:
					...
					for c2 in get_connection_list():
						if c2.from == c.from:
							...
							new_connections.push_back(
								{
									from = get_node(source.from).generator.name,
									from_port = source.from_port,
									to = get_node(c2.to).generator.name,
									to_port = c2.to_port
								}
							)
	...
	if !removed:
		...
		var reroute_node = {
			name = "reroute",
			type = "reroute",
			node_position = {x = reroute_position.x, y = reroute_position.y}
		}
		for c2 in get_connection_list():
			if c2.to == node.name and c2.to_port == port_index:
				...
				new_connections.push_back(
					{
						from = get_node(c2.from).generator.name,
						from_port = c2.from_port,
						to = "reroute",
						to_port = 0
					}
				)
		new_connections.push_back(
			{from = "reroute", from_port = 0, to = node.generator.name, to_port = port_index}
		)
...
func add_reroute_to_output(node: MMGraphNodeMinimal, port_index: int) -> void:
		...
		if c.from == node.name and c.from_port == port_index:
			...
			if to_node.generator is MMGenReroute:
				...
				for c2 in get_connection_list():
					if c2.from == c.to:
						do_disconnect_node(c2.from, c2.from_port, c2.to, c2.to_port)
						reroute_connections.push_back(
							{
								from = node.generator.name,
	...
	if !reroutes:
		...
		var reroute_connections = [
			{from = node.generator.name, from_port = port_index, to = "reroute", to_port = 0}
		]
			...
			reroute_connections.push_back(
				{
					from = "reroute",
					from_port = 0,
					to = get_node(d.to).generator.name,
					to_port = d.to_port
				}
			)
