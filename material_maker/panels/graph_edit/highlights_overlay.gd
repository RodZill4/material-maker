...
func _ready() -> void:
	graph.child_order_changed.connect(move_above_connections.call_deferred)
	...
	graph.gui_input.connect(graph_gui_input)
...
func move_above_connections() -> void:
	graph.move_child(self, graph.get_node("_connection_layer").get_index() + 1)

func graph_gui_input(event : InputEvent) -> void:
