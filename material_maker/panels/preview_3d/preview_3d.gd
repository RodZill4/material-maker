...
const MENU = [
	...
	{
		menu = "Model/Rotate",
		command = "set_rotate_model_speed",
		description = "Off",
		command_parameter = 0
	},
	{
		...
		command = "set_rotate_model_speed",
		description = "Slow",
		command_parameter = 0.01
	},
	{
		menu = "Model/Rotate",
		command = "set_rotate_model_speed",
		description = "Medium",
		command_parameter = 0.05
	},
	{
		menu = "Model/Rotate",
		command = "set_rotate_model_speed",
		description = "Fast",
		command_parameter = 0.1
	},
	{
		menu = "Model/Generate map",
		submenu = "generate_mesh_normal_map",
		description = "Mesh normal"
	},
...
func create_menu_model_list(menu: PopupMenu) -> void:
	...
	for i in objects.get_child_count():
		...
		var thumbnail := load(
			"res://material_maker/panels/preview_3d/thumbnails/meshes/%s.png" % o.name
		)
		if thumbnail:
...
func zoom(amount: float):
	camera.translation.z = clamp(
		camera.translation.z * amount, CAMERA_DISTANCE_MIN, CAMERA_DISTANCE_MAX
	)
...
func on_gui_input(event) -> void:
	if event is InputEventMouseButton:
		...
		if Input.is_key_pressed(KEY_ALT):
			...
			var objects_rotation: int = (
				-1
				if Input.is_key_pressed(KEY_CONTROL)
				else 1 if Input.is_key_pressed(KEY_SHIFT) else 0
			)
