	...
	if event is InputEventPanGesture:
		...
		match event.button_index:
			MOUSE_BUTTON_WHEEL_UP:
				...
				else: zoom(1.0 / (1.01 if event.shift_pressed else 1.1))
			MOUSE_BUTTON_WHEEL_DOWN:
				if event.is_command_or_control_pressed(): set_fov(parent.camera.fov - 1)
				else: zoom(1.01 if event.shift_pressed else 1.1)
			MOUSE_BUTTON_LEFT, MOUSE_BUTTON_RIGHT:
				...
				if event.pressed and lpressed != rpressed: # xor
					...
					mouse_start_position = event.global_position/parent.get_window().content_scale_factor
					moving = true
