...
func get_environment_list() -> Array:
	...
	for i in environments.size():
		...
		var item = {
			name = env.name if env.has("name") else "unnamed", thumbnail = env_textures.thumbnail
		}
		list.push_back(item)
	return list
...
func _physics_process(_delta) -> void:
	progress_window.set_progress(
		float($HTTPRequest.get_downloaded_bytes()) / float($HTTPRequest.get_body_size())
	)
...
func do_update_thumbnail() -> void:
	...
	for index in thumbnail_update_list:
		apply_environment(
			index,
			$PreviewGenerator/CameraPosition/CameraRotation1/CameraRotation2/Camera.environment,
			$PreviewGenerator/Sun
		)
