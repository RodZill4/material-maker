...
func load_obj_file(path: String) -> Mesh:
	...
	while !mdlFile.eof_reached():
		...
		if mdlData.begins_with("v "):
			...
			var vertex = Vector3(float(vertData[1]), float(vertData[2]), float(vertData[3]))

			mdlVerts.push_back(vertex)
		elif mdlData.begins_with("vt"):
			var uvData = mdlData.split(" ", false)
			var uv = Vector2(float(uvData[1]), 1.0 - float(uvData[2]))

			mdlUV.push_back(uv)
		elif mdlData.begins_with("vn"):
			var normData = mdlData.split(" ", false)
			var normal = Vector3(float(normData[1]), float(normData[2]), float(normData[3]))
			...
			if num == 6:
				...
				var faceIndices = Vector3(
					int(misc[2][0]) - 1, int(misc[1][0]) - 1, int(misc[0][0]) - 1
				)

				var uvIndices = Vector3(
					int(misc[2][1]) - 1, int(misc[1][1]) - 1, int(misc[0][1]) - 1
				)

				var normIndices = Vector3(
					int(misc[2][2]) - 1, int(misc[1][2]) - 1, int(misc[0][2]) - 1
				)
				...
				var faceIndices = Vector3(
					int(miscData[3]) - 1, int(miscData[2]) - 1, int(miscData[1]) - 1
				)
