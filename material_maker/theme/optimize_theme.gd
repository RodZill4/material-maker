...
func optimize_theme(theme_path):
	...
	for stylebox_name in theme.get_stylebox_types():
		for color_name in theme.get_color_list(stylebox_name):
			new_theme.set_color(
				color_name, stylebox_name, theme.get_color(color_name, stylebox_name)
			)
		for constant_name in theme.get_constant_list(stylebox_name):
			new_theme.set_constant(
				constant_name, stylebox_name, theme.get_constant(constant_name, stylebox_name)
			)
		for font_name in theme.get_font_list(stylebox_name):
			new_theme.set_font(
				font_name, stylebox_name, theme.get_font(font_name, stylebox_name).duplicate()
			)
		for sb_name in theme.get_stylebox_list(stylebox_name):
			...
			if stylebox is StyleBoxTexture:
				...
				new_theme.set_stylebox(
					sb_name, stylebox_name, theme.get_stylebox(sb_name, stylebox_name)
				)
