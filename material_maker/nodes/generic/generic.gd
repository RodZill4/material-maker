...
func _draw() -> void:
	...
	if generator != null and generator.preview >= 0 and get_connection_output_count() > 0:
		...
		draw_texture(
			preload("res://material_maker/icons/output_preview.tres"),
			conn_pos - Vector2(8, 8),
			get_color("title_color")
		)
...
static func update_control_from_parameter(parameter_controls: Dictionary, p: String, v) -> void:
	if parameter_controls.has(p):
		...
		if o is Control and o.filename == "res://material_maker/widgets/float_edit/float_edit.tscn":
			...
			and (
				o.filename
				== "res://material_maker/widgets/file_picker_button/file_picker_button.tscn"
			)
		):
			o.path = v
		elif (
			...
			and (
				o.filename
				== "res://material_maker/widgets/image_picker_button/image_picker_button.tscn"
			)
		):
			o.do_set_image_path(v)
		elif (
			o is Control
			and o.filename == "res://material_maker/widgets/gradient_editor/gradient_editor.tscn"
		):
			...
			o is Button
			and o.filename == "res://material_maker/widgets/curve_edit/curve_edit.tscn"
		):
			...
			o is Button
			and o.filename == "res://material_maker/widgets/polygon_edit/polygon_edit.tscn"
		):
...
func set_generator_parameter_ext(variable: String, value, old_value, merge_undo: bool = false):
	...
	if old_value != null and get_parent().get("undoredo") != null:
		...
		if old_value != serialized_value:
			...
			var undo_command = {
				type = "setparams", node = node_hier_name, params = {variable: old_value}
			}
			var redo_command = {
				type = "setparams", node = node_hier_name, params = {variable: serialized_value}
			}
			get_parent().undoredo.add(
				"Set parameter value", [undo_command], [redo_command], merge_undo
			)
...
static func get_parameter_tooltip(p: Dictionary, parameter_value = null) -> String:
	...
	if p.has("shortdesc"):
		tooltip = (
			TranslationServer.translate(p.shortdesc)
			+ " ("
			+ TranslationServer.translate(p.name)
			+ ")"
		)
...
static func create_parameter_control(p: Dictionary, accept_float_expressions: bool) -> Control:
	...
	if p.type == "float":
		...
		control.set_script(
			preload("res://material_maker/widgets/color_picker_button/color_picker_button.gd")
		)
...
func update_node() -> void:
	...
	for i in range(output_count):
		...
		set_slot(
			i,
			is_slot_enabled_left(i),
			get_slot_type_left(i),
	...
	if generator.is_editable():
		...
		set_slot(
			edit_buttons.get_index(), false, 0, Color(0.0, 0.0, 0.0), false, 0, Color(0.0, 0.0, 0.0)
		)
	if generator.minimized:
...
func finalize_generator_update() -> void:
	if !edit_generator_next_state.empty():
		get_parent().undoredo_create_step(
			"Edit node",
			generator.get_parent().get_hier_name(),
