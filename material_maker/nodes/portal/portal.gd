...
func is_link_unique(link := get_link()) -> bool:
	if is_portal_in():
...
func set_unique_portal_link(force := false) -> void:
	if (is_portal_in() and not is_link_unique()) or force:
...
func link_collision_warning_color(link := get_link()) -> Color:
