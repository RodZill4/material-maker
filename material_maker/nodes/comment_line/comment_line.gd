...
func on_label_context_menu(id : int) -> void:
	var old_value : int = generator.get_parameter("label")
	if old_value != id and get_parent().get("undoredo") != null:
		var node_hier_name = generator.get_hier_name()
		var undo_command : Dictionary = { type="setparams", node=node_hier_name, params={ "label"=old_value } }
		var redo_command : Dictionary = { type="setparams", node=node_hier_name, params={ "label"=id } }
