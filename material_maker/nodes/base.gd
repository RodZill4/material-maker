...
func _draw() -> void:
	...
	if (
		generator != null
		and generator.model == null
		and (generator is MMGenShader or generator is MMGenGraph)
	):
...
# warning-ignore:narrowing_conversion
			var conn_pos2 = get_connection_input_position(
				min(i + inputs[i].group_size - 1, inputs.size() - 1)
			)
			conn_pos1 /= scale
			conn_pos2 /= scale
		...
		if show_inputs:
			var string: String = (
				TranslationServer.translate(inputs[i].shortdesc)
				if inputs[i].has("shortdesc")
				else TranslationServer.translate(inputs[i].name)
			)
			var string_size: Vector2 = font.get_string_size(string)
			draw_string(
				font,
				(
					get_connection_input_position(i) / scale
					- Vector2(string_size.x + 12, -string_size.y * 0.3)
				),
				string,
				color
			)
	...
	for i in range(2):
		if (
			get_parent().locked_preview[i] != null
			and get_parent().locked_preview[i].generator == generator
		):
			preview_port[i] = get_parent().locked_preview[i].output_index
			preview_locked[i] = true
		elif (
			get_parent().current_preview[i] != null
			and get_parent().current_preview[i].generator == generator
		):
...
# warning-ignore:narrowing_conversion
			...
			var conn_pos2 = get_connection_output_position(
				min(i + outputs[i].group_size - 1, outputs.size() - 1)
			)
			conn_pos1 /= scale
			conn_pos2 /= scale
			draw_line(conn_pos1, conn_pos2, color)
		...
		if j != -1:
			...
			conn_pos /= scale
			draw_texture_rect(
				PREVIEW_LOCKED_ICON if preview_locked[j] else PREVIEW_ICON,
				Rect2(conn_pos.x - 14, conn_pos.y - 4, 7, 7),
				false,
				color
			)
		if show_outputs:
			var string: String = (
				TranslationServer.translate(outputs[i].shortdesc)
				if outputs[i].has("shortdesc")
				else (tr("Output") + " " + str(i))
			)
			var string_size: Vector2 = font.get_string_size(string)
			draw_string(
				font,
				get_connection_output_position(i) / scale + Vector2(12, string_size.y * 0.3),
				string,
				color
			)
	if selected:
...
func set_generator_seed(s: float):
	...
	get_parent().undoredo.add(
		"Set seed",
		[{type = "setseed", node = hier_name, seed = old_seed}],
		[{type = "setseed", node = hier_name, seed = s}],
		false
	)
...
func _on_seed_menu(id):
	match id:
		0:
			...
			get_parent().undoredo.add(
				"Lock/unlock seed",
				[{type = "setseedlocked", node = hier_name, seedlocked = old_seed_locked}],
				[{type = "setseedlocked", node = hier_name, seedlocked = !old_seed_locked}],
				false
			)
		1:
	...
	if event is InputEventMouseButton and event.pressed:
		if Rect2(rect_size.x - 40, 4, 16, 16).has_point(event.position):
			if event.button_index == BUTTON_LEFT:
				...
				get_parent().undoredo.add(
					"Minimize node",
					[{type = "setminimized", node = hier_name, minimized = !generator.minimized}],
					[{type = "setminimized", node = hier_name, minimized = generator.minimized}],
					false
				)
				update_node()
			...
			match event.button_index:
				...
				BUTTON_RIGHT:
					...
					menu.add_item(
						tr("Unlock seed") if generator.is_seed_locked() else tr("Lock seed"), 0
					)
					menu.add_separator()
...
func get_slot_tooltip(pos: Vector2) -> String:
	...
	if get_connection_input_count() > 0:
		...
		var input_2: Vector2 = (
			get_connection_input_position(get_connection_input_count() - 1)
			+ 5 * scale
		)
	...
	if get_connection_output_count() > 0:
		...
		var output_2: Vector2 = (
			get_connection_output_position(get_connection_output_count() - 1)
			+ 5 * scale
		)
...
func _on_menu_id_pressed(id: int) -> void:
	match id:
		MENU_PROPAGATE_CHANGES:
			...
			dialog.dialog_text = (
				"Propagate changes from %s to %d nodes?"
				% [
					generator.get_type_name(),
					get_parent().get_propagation_targets(generator).size()
				]
			)
