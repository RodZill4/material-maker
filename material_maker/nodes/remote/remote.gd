...
func update_node() -> void:
	...
	for i in range(parameter_count):
		...
		if control != null:
			...
			add_control(
				generator.get_widget(p.name).label,
				control,
				is_named_param,
				shortdesc,
				longdesc,
				...
				match command:
					1:
						...
						var status = dialog.enter_text(
							"Configuration", "Enter a name for the new configuration", ""
						)
...
func on_param_name_changed(new_name: String, param_name: String, line_edit: LineEdit) -> void:
	if generator.rename(param_name, new_name, true):
		line_edit.add_color_override(
			"font_color", get_node("/root/MainWindow").theme.get_color("font_color", "LineEdit")
		)
	else:
	...
	for p in generator.get_parameter_defs():
		if p.name == param_name:
			...
			if result.keys().size() == 4:
				old_state = generator.serialize().duplicate(true)
				generator.configure_named_parameter(
					param_name, result.min, result.max, result.step, result.default
				)
