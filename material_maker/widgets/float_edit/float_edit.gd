	...
	if update_text and $Slider.visible:
		...
		if cursor != null:
			if max_value != min_value:
				cursor.rect_position.x = (
					(clamp(value, min_value, max_value) - min_value)
					* (slider.rect_size.x - cursor.rect_size.x)
					/ (max_value - min_value)
				)
			else:
...
func _gui_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT:
		...
		if new_modifiers != modifiers:
			...
			var v: float = (
				start_value
				+ sign(delta) * pow(abs(delta) * 0.005, 2) * abs(max_value - min_value)
			)
			if current_step != 0:
