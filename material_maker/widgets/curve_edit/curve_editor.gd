...
func _on_ControlPoint_moved(index):
	...
	if control_point.has_node("LeftSlope"):
		...
		if slope_vector.x != 0:
			...
			%LeftSlope.value = snapped(-rad_to_deg(atan(curve.points[index].ls)), 0.01)
	...
	if control_point.has_node("RightSlope"):
		...
		if slope_vector.x != 0:
			...
			%RightSlope.value = snapped(rad_to_deg(atan(curve.points[index].rs)), 0.01)
	update_control_ui()
	queue_redraw()
	emit_signal("value_changed", curve)
...
func update_control_ui():
	%ControlUI.show()
	%LeftSlope.value = snapped(-rad_to_deg(atan(curve.points[point_index].ls)), 0.01)
	%RightSlope.value = snapped(rad_to_deg(atan(curve.points[point_index].rs)), 0.01)
	%PositionY.value = snapped(curve.points[point_index].p.y, 1e-3)
	if point_index == 0 or point_index == curve.points.size()-1:
		...
		%PositionX.value = snapped(curve.points[point_index].p.x, 1e-3)
...
func _on_left_slope_value_changed(value: Variant) -> void:
	if point_index != 0:
		var rad = deg_to_rad(clamp(value, -90+1e-3, 90-1e-3))
		curve.points[point_index].ls = snapped(-tan(rad), 0.01)
	update_controls()
	queue_redraw()
	emit_signal("value_changed", curve)
...
func _on_right_slope_value_changed(value: Variant) -> void:
	if point_index != curve.points.size() - 1:
		var rad = deg_to_rad(clamp(value, -90+1e-3, 90-1e-3))
		curve.points[point_index].rs = snapped(tan(rad), 0.01)
	update_controls()
	queue_redraw()
	emit_signal("value_changed", curve)
