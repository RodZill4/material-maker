	...
	for i in curve.points.size():
		...
		if i == 0 or i == curve.points.size() - 1:
			control_point.set_constraint(
				control_point.rect_position.x,
				control_point.rect_position.x,
				-control_point.OFFSET.y,
				rect_size.y - control_point.OFFSET.y
			)
			if i == 0:
			...
			control_point.set_constraint(
				min_x, max_x, -control_point.OFFSET.y, rect_size.y - control_point.OFFSET.y
			)
...
func _on_ControlPoint_moved(index):
	...
	curve.points[index].p = reverse_transform_point(
		control_point.rect_position + control_point.OFFSET
	)
