...
const MENU = [
	...
	{
		menu = "File",
		command = "new_paint_project",
		shortcut = "Control+Shift+N",
		description = "New paint project"
	},
	{menu = "File", command = "load_project", shortcut = "Control+O", description = "Load"},
	{
		menu = "File",
		command = "load_material_from_website",
		description = "Load material from website"
	},
	...
	{
		menu = "Edit",
		command = "edit_select_none",
		shortcut = "Control+Shift+A",
		description = "Select None"
	},
	{
		menu = "Edit",
		command = "edit_select_invert",
		shortcut = "Control+I",
	...
	{
		menu = "Tools",
		command = "create_subgraph",
		shortcut = "Control+G",
		description = "Create group"
	},
	{
		menu = "Tools",
		command = "make_selected_nodes_editable",
		shortcut = "Control+W",
		description = "Make selected nodes editable"
	},
	{menu = "Tools"},
	{
		menu = "Tools",
		submenu = "add_selection_to_library",
		description = "Add selected node to library",
		mode = "material"
	},
	{
		menu = "Tools",
		submenu = "add_brush_to_library",
		description = "Add current brush to library",
		mode = "paint"
	},
	{
		menu = "Tools",
		command = "generate_graph_screenshot",
		description = "Create a screenshot of the current graph",
		mode = "material"
	},
	{
		menu = "Tools",
		command = "paint_project_settings",
		description = "Paint project settings",
		mode = "paint"
	},
	{
		menu = "Tools",
		submenu = "paint_environment",
		description = "Set painting environment",
		mode = "paint"
	},
	{menu = "Tools"},
	...
	{
		menu = "Help",
		command = "show_library_item_doc",
		shortcut = "Control+F1",
		description = "Show selected library item documentation"
	},
...
func _ready() -> void:
	...
	OS.set_window_title(
		(
			...
			+ " v"
			+ ProjectSettings.get_setting("application/config/actual_release")
		)
	)
...
func on_config_changed() -> void:
	...
	OS.low_processor_usage_mode_sleep_usec = (
		(1.0 / clamp(get_config("fps_limit"), FPS_LIMIT_MIN, FPS_LIMIT_MAX))
		* 1_000_000
	)
	# locale
	...
	get_tree().set_screen_stretch(
		SceneTree.STRETCH_MODE_DISABLED, SceneTree.STRETCH_ASPECT_IGNORE, Vector2(), scale
	)
...
func create_menu(menu_def: Array, object: Object, menu: PopupMenu, menu_name: String) -> PopupMenu:
	...
	for i in menu_def.size():
		if (
			menu_def[i].has("standalone_only")
			and menu_def[i].standalone_only
			and Engine.editor_hint
		):
			continue
	...
	if !menu.is_connected("about_to_show", self, "on_menu_about_to_show"):
		menu.connect(
			"about_to_show", self, "on_menu_about_to_show", [menu_def, object, menu_name, menu]
		)
	return menu
...
func _on_ExportMaterial_id_pressed(id) -> void:
	...
	dialog.add_filter(
		"*." + material_node.get_export_extension(profile) + ";" + profile + " Material"
	)
...
func new_paint_project(obj_file_name = null) -> void:
	...
	paint_panel.init_project(
		result.mesh, result.mesh_filename, result.size, result.project_filename
	)
...
func add_selection_to_library(index) -> void:
	...
	var status = dialog.enter_text(
		"New library element", "Select a name for the new library element", current_item_name
	)
...
func add_brush_to_library(index) -> void:
	...
	var status = dialog.enter_text(
		"New library element",
		"Select a name for the new library element",
		brushes.get_selected_item_name()
	)
	while status is GDScriptFunctionState:
		status = yield(status, "completed")
	if !status.ok:
...
func environment_editor() -> void:
	add_child(
		load("res://material_maker/windows/environment_editor/environment_editor.tscn").instance()
	)
...
func show_library_item_doc_is_disabled() -> bool:
	return (
		get_doc_dir() == ""
		or !library.is_inside_tree()
		or library.get_selected_item_doc_name() == ""
	)
...
func update_preview_3d(previews: Array, sequential = false) -> void:
	...
	if (
		graph_edit != null
		and graph_edit.top_generator != null
		and graph_edit.top_generator.has_node("Material")
	):
...
func _notification(what: int) -> void:
	match what:
		MainLoop.NOTIFICATION_WM_FOCUS_OUT:
			...
			OS.low_processor_usage_mode_sleep_usec = (
				(1.0 / clamp(get_config("idle_fps_limit"), IDLE_FPS_LIMIT_MIN, IDLE_FPS_LIMIT_MAX))
				* 1_000_000
			)
...
func generate_graph_screenshot():
	...
	graph_edit_rect = Rect2(
		graph_edit_rect.position + Vector2(15, 80), graph_edit_rect.size - Vector2(30, 180)
	)
	var graph_rect = null
	for c in graph_edit.get_children():
		if c is GraphNode:
	...
	small_image.create(
		graph_edit_rect.size.x,
		graph_edit_rect.size.y,
		false,
		get_viewport().get_texture().get_data().get_format()
	)
	for x in range(0, graph_rect.size.x, graph_edit_rect.size.x):
		for y in range(0, graph_rect.size.y, graph_edit_rect.size.y):
			...
			yield(timer, "timeout")
			timer.queue_free()
			small_image.blit_rect(
				get_viewport().get_texture().get_data(), graph_edit_rect, Vector2(0, 0)
			)
			small_image.flip_y()
