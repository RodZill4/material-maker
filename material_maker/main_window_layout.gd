...
const PANELS = [
	{
		name = "Library",
		scene = preload("res://material_maker/panels/library/library.tscn"),
		position = "TopLeft"
	},
	{
		name = "Preview2D",
		scene = preload("res://material_maker/panels/preview_2d/preview_2d_panel.tscn"),
		position = "BottomLeft"
	},
	{
		name = "Preview3D",
		scene = preload("res://material_maker/panels/preview_3d/preview_3d_panel.tscn"),
		position = "BottomLeft"
	},
	{
		name = "Preview2D (2)",
	...
	{
		name = "Histogram",
		scene = preload("res://material_maker/widgets/histogram/histogram.tscn"),
		position = "BottomLeft"
	},
	{
		name = "Hierarchy",
		scene = preload("res://material_maker/panels/hierarchy/hierarchy_panel.tscn"),
		position = "TopRight"
	},
	{
		name = "Reference",
		scene = preload("res://material_maker/panels/reference/reference_panel.tscn"),
		position = "BottomLeft"
	},
	{
		name = "Brushes",
		scene = preload("res://material_maker/panels/brushes/brushes.tscn"),
		position = "TopLeft"
	},
	{
		name = "Layers",
		scene = preload("res://material_maker/panels/layers/layers.tscn"),
		position = "BottomRight"
	},
	{
		name = "Parameters",
		scene = preload("res://material_maker/panels/parameters/parameters.tscn"),
		position = "TopRight"
	},
]
const HIDE_PANELS = {
	material = ["Brushes", "Layers", "Parameters"], paint = ["Preview3D", "Histogram", "Hierarchy"]
}
...
func load_panels(config_cache) -> void:
	...
	for panel in PANELS:
		...
		if config_cache.has_section_key("layout", config_panel_name + "_location"):
			tab = get_node(
				PANEL_POSITIONS[config_cache.get_value("layout", config_panel_name + "_location")]
			)
		if (
			config_cache.has_section_key("layout", config_panel_name + "_hidden")
			&& config_cache.get_value("layout", config_panel_name + "_hidden")
		):
...
func update_panels() -> void:
	...
	if $SplitRight/Right/Top.get_tab_count() == 0:
		if $SplitRight/Right/Bottom.get_tab_count() == 0:
			right_requested = 10
			$SplitRight/Right.split_offset -= (
				($SplitRight/Right/Top.rect_size.y - $SplitRight/Right/Bottom.rect_size.y)
				/ 2
			)
