...
func _on_ItemList_item_activated(index) -> void:
	...
	var error = $HTTPRequest.request(
		"https://www.materialmaker.org/api/getMaterial?id=" + str(m.id)
	)
	if error != OK:
		return
...
func update_thumbnails() -> void:
	...
	for i in range(materials.size()):
		...
		if image.load(cache_filename) != OK:
			var error = $ImageHTTPRequest.request(
				"https://www.materialmaker.org/data/materials/material_" + str(m.id) + ".webp"
			)
			if error == OK:
