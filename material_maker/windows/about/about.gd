...
func _on_patrons_gui_input(event : InputEvent) -> void:
	mm_globals.handle_vscroll_gesture(event, %Patrons, %Patrons.get_v_scroll_bar(), 0,
			%Patrons.get_rect().size.y)

func _on_authors_gui_input(event : InputEvent) -> void:
	mm_globals.handle_vscroll_gesture(event, %Authors, %Authors.get_v_scroll_bar(), 0,
			%Authors.get_rect().size.y)

func _on_license_gui_input(event : InputEvent) -> void:
	mm_globals.handle_vscroll_gesture(event, %License, %License.get_v_scroll_bar(), 0,
