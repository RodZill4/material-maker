...
func select_export(i: int) -> void:
	...
	export_extension_edit.text = (
		exports[e].export_extension
		if exports[e].has("export_extension")
		else ""
	)
	update_files(e)
...
func select_file(i: int) -> void:
	...
	if i == exports[e].files.size():
		exports[e].files.push_back(
			{file_name = "$(path_prefix)_foo.png", type = "texture", output = 0}
		)
		export_files.add_item("Add file...")
		...
		match f.type:
			...
			"template", "buffer_templates":
				...
				if f.has("file_params"):
					...
					f.erase("file_params")
				var template: String = GEN_MATERIAL.process_template(
					GEN_MATERIAL.get_template_text(f.template), file_export_context
				)
