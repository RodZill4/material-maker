...
func set_source(g, o):
	...
	var anim_code = code
	anim_code = anim_code.replace(
		"varying float elapsed_time;",
		"uniform float begin;\nuniform float end;\nvarying float elapsed_time;"
	)
	anim_code = anim_code.replace(
		"elapsed_time = TIME;",
		"elapsed_time = (begin == end) ? begin : begin+sign(end-begin)*mod(TIME, abs(end-begin));"
	)
...
func _on_Export_pressed():
	...
	if files.size() > 0:
		...
		if spritesheet_lines != 0:
			...
			spritesheet.create(
				size * spritesheet_columns, size * spritesheet_lines, false, Image.FORMAT_RGBA8
			)
		else:
			...
			if regex_match.size() > 1:
				...
				filename_fmt = filename.replace(
					regex_match[0].strings[0], "%0" + str(regex_match[0].strings[0].length()) + "d"
				)
			else:
		...
		for i in range(0, images):
			...
			if spritesheet_lines > 0:
				...
				spritesheet.blit_rect(
					image,
					Rect2(0, 0, size, size),
					Vector2(size * (i % spritesheet_columns), size * (i / spritesheet_columns))
				)
			else:
