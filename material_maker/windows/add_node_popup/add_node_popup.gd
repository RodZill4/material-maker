...
func show_popup(
	node_name: String = "", slot: int = -1, slot_type: int = -1, is_output: bool = false
) -> void:
		...
		if ref_obj.has("shader_model"):
			if !ref_obj.shader_model.has("outputs") or ref_obj.shader_model.outputs.empty():
				...
				for outputs in ref_obj.shader_model.outputs.size():
					if (
						mm_io_types.types[ref_obj.shader_model.outputs[outputs].type].slot_type
						== qc_slot_type
					):
						found = true
						break
			...
			if (
				output_ports.empty()
				or mm_io_types.types[output_ports[0].type].slot_type != qc_slot_type
			):
				return false
		elif (
			(ref_obj.type == "image" or ref_obj.type == "text" or ref_obj.type == "buffer")
			and qc_slot_type != 0
		):
			return false
	else:
		if ref_obj.has("shader_model"):
			if !ref_obj.shader_model.has("inputs") or ref_obj.shader_model.inputs.empty():
				...
				for input in ref_obj.shader_model.inputs.size():
					if (
						mm_io_types.types[ref_obj.shader_model.inputs[input].type].slot_type
						== qc_slot_type
					):
						found = true
						break
			...
			if (
				input_ports.empty()
				or mm_io_types.types[input_ports[0].type].slot_type != qc_slot_type
			):
				return false
		elif ref_obj.type == "image" or ref_obj.type == "text":
			return false
		elif (
			(ref_obj.type == "debug" or ref_obj.type == "buffer" or ref_obj.type == "export")
			and qc_slot_type != 0
		):
			return false
	return true
