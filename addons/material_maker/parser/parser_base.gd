	...
	while true:
		if !actions[state].has(next_token.type):
			return {
				...
				msg = (
					"near '%s' (expected '%s')"
					% [next_token.value, PoolStringArray(actions[state].keys()).join("', '")]
				),
				pos = next_token.pos_begin
			}
		...
		match action[0]:
			"s":
				...
				if tokens.empty():
					return {
						status = "ERROR", msg = "Reached end of file", pos = next_token.pos_end + 1
					}
			...
			"r":
				...
				if has_method(rule.function):
					...
					if value == null:
						return {
							status = "ERROR",
							msg = "Error while building token",
							pos = next_token.pos_end + 1
						}
