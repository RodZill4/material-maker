...
func get_parameter_defs():
	return [
		...
		{ label="H", name="h_mirror", type="boolean", default=true },
		{ label="V", name="v_mirror", type="boolean", default=true }
	]
...
func mod_uv_code(scene : Dictionary, output_name : String) -> String:
	var code : String = ""
	if scene.parameters.h_mirror:
		code += "%s_p.x = abs(%s_p.x);\n" % [ output_name, output_name ]
	if scene.parameters.v_mirror:
