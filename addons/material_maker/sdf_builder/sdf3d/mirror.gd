...
func get_parameter_defs():
	return [
		...
		{ label="X", name="x_mirror", type="boolean", default=true },
		{ label="Y", name="y_mirror", type="boolean", default=true },
		{ label="Z", name="z_mirror", type="boolean", default=true }
	]
...
func mod_uv_code(scene : Dictionary, output_name : String) -> String:
	var code : String = ""
	if scene.parameters.x_mirror:
		code += "%s_p.x = abs(%s_p.x);\n" % [ output_name, output_name ]
	if scene.parameters.y_mirror:
		code += "%s_p.y = abs(%s_p.y);\n" % [ output_name, output_name ]
	if scene.parameters.z_mirror:
