...
func find_instance_functions(code: String):
	...
	for r in result:
		if not r.strings[1] in ["return"]:
			...
			code = code.replace(
				r.strings[0],
				"%s %s(%s, float _seed_variation_) {" % [r.strings[1], r.strings[2], r.strings[3]]
			)
...
func preprocess_shader_model(data: Dictionary):
	...
	if data.has("instance") and data.instance != "":
		...
		preprocessed.instance = fix_instance_functions(
			instance_functions.code, instance_functions.functions
		)
		if data.has("code"):
			...
			for p in data.parameters:
				if p.type == "enum":
					...
					for i in p.values.size():
						var v = fix_instance_functions(
							p.values[i].value, instance_functions.functions
						)
...
func replace_input_with_function_call(
	string: String,
	input: String,
	seed_parameter: String = ", _seed_variation_",
	input_suffix: String = ""
) -> String:
	...
	while true:
		...
		string = string.replace(
			"$%s(%s)" % [input + input_suffix, uv],
			"%s_input_%s(%s%s)" % [genname, input, uv, seed_parameter]
		)
	return string


func replace_input(
	string: String, context, input: String, type: String, src: OutputPort, default: String
) -> Dictionary:
	...
	return {
		...
		defs = required_defs,
		code = required_code,
		textures = required_textures,
		pending_textures = required_pending_textures,
...
func subst(string: String, context: MMGenContext, uv: String = "") -> Dictionary:
	...
	if (
		shader_model_preprocessed.has("parameters")
		and typeof(shader_model_preprocessed.parameters) == TYPE_ARRAY
	):
		...
		for p in shader_model_preprocessed.parameters:
			...
			if p.type == "float":
				...
				value_string = (
					"vec4(p_%s_%s_r, p_%s_%s_g, p_%s_%s_b, p_%s_%s_a)"
					% [genname, p.name, genname, p.name, genname, p.name, genname, p.name]
				)
		...
		while cont:
			...
			for i in range(shader_model.inputs.size()):
				...
				if input.has("function") and input.function:
					...
					var result = replace_input(
						string, context, input.name, input.type, source, input.default
					)
	...
	if shader_model.has("inputs"):
		for i in range(shader_model.inputs.size()):
			...
			if input.has("function") and input.function:
				...
				var result = replace_input(
					string, local_context, input.name, input.type, source, input.default
				)
				assert(!(result is GDScriptFunctionState))
				while result is GDScriptFunctionState:
					result = yield(result, "completed")
				...
				rv.defs += (
					"%s %s_input_%s(%s, float _seed_variation_) {\n"
					% [
						mm_io_types.types[input.type].type,
						genname,
						input.name,
						mm_io_types.types[input.type].paramdefs
					]
				)
				rv.defs += "%s\n" % result.code
				rv.defs += "return %s;\n}\n" % result.string
...
func _get_shader_code(uv: String, output_index: int, context: MMGenContext) -> Dictionary:
	...
	if (
		shader_model_preprocessed != null
		and shader_model_preprocessed.has("outputs")
		and shader_model_preprocessed.outputs.size() > output_index
	):
