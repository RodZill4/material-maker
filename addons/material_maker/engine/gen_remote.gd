		...
		if !widget.empty():
			match widget.type:
				...
				"named_parameter":
					...
					if is_inside_tree():
						get_tree().call_group(
							"preview", "on_float_parameters_changed", {param_name: v}
						)
	.set_parameter(p, v)
...
func create_config_control(label: String) -> String:
	var n = get_next_widget_name()
	widgets.push_back(
		{name = n, label = label, type = "config_control", linked_widgets = [], configurations = {}}
	)
	return n


func create_named_parameter(
	label: String,
...
func configure_named_parameter(
	parameter_name: String,
	minimum: float = 0.0,
	maximum: float = 1.0,
	step: float = 0.01,
