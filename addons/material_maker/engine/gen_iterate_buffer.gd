...
func on_float_parameters_changed(parameter_changes: Dictionary) -> bool:
	...
	var not_just_iteration = (
		parameter_changes.size() > 1
		or not parameter_changes.has("o%s_iteration" % str(get_instance_id()))
	)
	...
	for i in range(2):
		...
		if (
			(mm_renderer.update_float_parameters(m, parameter_changes) or iterations_changed)
			and not_just_iteration
		):
			update_again = true
			return_value = true
			set_current_iteration(0)
...
func on_texture_invalidated(n: String) -> void:
	for i in range(2):
		...
		if mm_renderer.material_has_parameter(m, n):
			if pending_textures[i].empty():
				get_tree().call_group(
					"preview", "on_texture_invalidated", "o%s_tex" % str(get_instance_id())
				)
				get_tree().call_group(
					"preview", "on_texture_invalidated", "o%s_loop_tex" % str(get_instance_id())
				)
				set_pending()
...
func set_current_iteration(i: int) -> void:
	...
	if is_inside_tree():
		get_tree().call_group(
			"preview", "on_float_parameters_changed", {iteration_param_name: current_iteration}
		)
	...
	if !updating:
		...
		while update_again:
			...
			if current_iteration == 0:
				...
				renderer = renderer.render_material(
					self, loop_material, pow(2, get_parameter("size"))
				)
			while renderer is GDScriptFunctionState:
				renderer = yield(renderer, "completed")
			if renderer == null:
		...
		if current_iteration <= iterations:
			get_tree().call_group(
				"preview", "on_texture_changed", "o%s_loop_tex" % str(get_instance_id())
			)
		else:
			get_tree().call_group(
				"preview", "on_texture_changed", "o%s_tex" % str(get_instance_id())
			)
		updating = false


func get_globals(texture_name: String) -> Array:
	var texture_globals: String = (
...
func _get_shader_code(uv: String, output_index: int, context: MMGenContext) -> Dictionary:
	var shader_code = _get_shader_code_lod(
		uv, output_index, context, -1.0, "_tex" if output_index == 0 else "_loop_tex"
	)
