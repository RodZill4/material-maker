...
func _serialize_data(data: Dictionary) -> Dictionary:
	...
	image_path = image_path.replace(
		OS.get_executable_path().get_base_dir(), "%MATERIAL_MAKER_PATH%"
	)
...
func _deserialize(data: Dictionary) -> void:
	if data.has("parameters") and data.parameters.has("image"):
		data.parameters.image = data.parameters.image.replace(
			"%PROJECT_PATH%", mm_loader.current_project_path
		)
		data.parameters.image = data.parameters.image.replace(
			"%USER_DATA_PATH%", OS.get_user_data_dir()
		)
		data.parameters.image = data.parameters.image.replace(
			"%MATERIAL_MAKER_PATH%", OS.get_executable_path().get_base_dir()
		)
	elif data.has("image"):
