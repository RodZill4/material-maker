	...
	match status:
		...
		ResourceLoader.THREAD_LOAD_LOADED:
			...
			if tween:
				tween.kill()
			tween = get_tree().create_tween()
			tween.tween_property(progress_bar, "value", 80.0, 1.0).set_trans(Tween.TRANS_LINEAR)

			tween.tween_property(progress_bar, "value", 100.0, 0.8).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
			tween.parallel().tween_property($SplashFade, "color", Color.BLACK, 0.7).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT).set_delay(0.2)
